...
const { jogoAdiado, jogoCancelado, jogoAceitaPalpite } = await import("./clubes.js");
const esperado = {
...
/* ── 10. jogoAceitaPalpite — mesma regra usada pela tela de Palpites e pelo
   servidor. "Sem data = sem palpite" protege a antecedência média (5º
   desempate) de palpite gravado antes de existir prazo. ── */
const comData = iso(AGORA + 3 * H);
checa("jogo com data e sem status: aceita", jogoAceitaPalpite({ kickoff: comData, status: null }));
checa("jogo com data e status TIMED: aceita", jogoAceitaPalpite({ kickoff: comData, status: "TIMED" }));
checa("jogo com data mas ADIADO: recusa", !jogoAceitaPalpite({ kickoff: comData, status: "POSTPONED" }),
  "adiado com data residual da API nao pode reabrir palpite");
checa("jogo ADIADO sem data: recusa", !jogoAceitaPalpite({ kickoff: null, status: "POSTPONED" }));
checa("jogo CANCELADO: recusa", !jogoAceitaPalpite({ kickoff: comData, status: "CANCELLED" }));
checa("jogo SEM DATA e sem status: recusa (cadastro manual do admin)",
  !jogoAceitaPalpite({ kickoff: null, status: null }),
  "era a brecha: palpite sem prazo rendia antecedencia enorme quando a data saisse");
checa("jogo ja iniciado NAO e barrado por aqui (quem barra e api/palpite.js)",
  jogoAceitaPalpite({ kickoff: iso(AGORA - 1 * H), status: "IN_PLAY" }));
checa("objeto vazio/undefined nao quebra", !jogoAceitaPalpite({}) && !jogoAceitaPalpite(undefined));

/* ── 11. config ausente/corrompida não derruba a rota ── */
