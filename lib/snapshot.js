...
import { jogoCancelado } from "./clubes.js";
...
async function construirSnapshot() {
  const [participantes, jogos, palpites, palpitesCampeao, palpitesArtilheiro, resultadoEspecial, premiados, reacoes, config] =
    await Promise.all([
      ...
      sql`SELECT id, casa, fora, kickoff, gh, ga, rodada, peso, live, status FROM jogos ORDER BY kickoff NULLS LAST, id`,
...
export function montarEstado(snap, eu, { prazoBonus, prazoPagamento }) {
  ...
  /* Jogo CANCELADO não existe pro app: some da lista, do ranking e das
     contagens, pra todo mundo — inclusive pro admin. Some aqui, no servidor,
     em vez de em cada tela, pra não sobrar nenhum caminho que ainda o mostre.
     ADIADO continua aparecendo: ele ainda vai valer, só está sem data. */
  const jogos = snap.jogos.filter((j) => !jogoCancelado(j));
  ...
  const palpites = snap.palpites
    .filter((p) => {
      /* palpite de jogo cancelado é descartado antes de qualquer outra regra,
         senão sobraria órfão apontando pra um jogo que a tela não recebeu. */
  ...
  for (const p of snap.palpites) {
    if (!jogoPorId.has(p.jogo_id)) continue;
    totPorJogo.set(p.jogo_id, (totPorJogo.get(p.jogo_id) || 0) + 1);
  }
